...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  String deliveryType = 'Selecciona una opción de envío';

  List<DropdownMenuItem<StatusOption>> _countryItems;
  StatusOption _selectedCountry;

  @override
  void initState() {
    List<StatusOption> countries = StatusOption.allStatus;

    // Initialise your items only once
    _countryItems = countries.map<DropdownMenuItem<StatusOption>>(
      (StatusOption countryOption) {
      ...
      body: Consumer<OrderProvider>(builder: (context, order, child) {
        ...
        if (widget.orderType == 'POS') {
          if (widget.extraDiscountType == 'percent' ||
              widget.extraDiscountType == 'Porcentaje') {
                                              ...
                                              Container(
                                                ...
                                                onChanged: (String newValue) {
                                                  setState(() {
                                                    ...
                                                    print(
                                                        '==========selected type===>${deliveryType.toString()}');
                                                  });
                                                },
                                                items: <String>[
                                  ...
                                  children: [
                                    Text(
                                        ...
                                        style: titilliumSemiBold.copyWith(
                                            fontSize: 14)),
                                    Text(
                                      '${getTranslated('name', context)} : ${widget.orderModel.customer != null ? widget.orderModel.customer.fName ?? '' : ""} ${widget.orderModel.customer != null ? widget.orderModel.customer.lName ?? '' : ""}',
                                      style: titilliumRegular,
                                    ),
                                    Text(
                                        '${getTranslated('address', context)} : ${widget.orderModel.shippingAddressData != null ? jsonDecode(widget.orderModel.shippingAddressData)['address'] : widget.orderModel.shippingAddress ?? ''}',
                                        style: titilliumRegular),
                                    Text(
                                        '${getTranslated('billing_address', context)} : ${widget.orderModel.billingAddressData != null ? widget.orderModel.billingAddressData.address : widget.orderModel.billingAddress ?? ''}',
                                        style: titilliumRegular),
                                    Text(
                                        '${getTranslated('phone', context)} : ${widget.orderModel.customer != null ? widget.orderModel.customer.phone ?? '' : ""}',
                                        style: titilliumRegular),
                                    Text(
                                        '${getTranslated('order_note', context)} : ${widget.orderModel.orderNote != null ? widget.orderModel.orderNote ?? '' : ""}',
                                        style: titilliumRegular),
                                  ]),
                            )
                          : SizedBox(),
                                          ...
                                          children: [
                                            Text(
                                                ...
                                                style: titilliumBold.copyWith(
                                                    ...
                                                    fontSize: 14)),
                                            Text(
                                                '${getTranslated('email', context)} : ${widget.orderModel.customer.email ?? ''}',
                                                style:
                              ...
                              child: widget.orderType == 'POS'
                                  ...
                                  : Consumer<OrderProvider>(
                                      ...
                                      return Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text('Actualiza el estado del pedido.', style: TextStyle(color: Colors.grey.shade800),),
                                          Row(
                                            children: [
                                              Expanded(
                                                child: Container(
...
class StatusOption {
  ...
  StatusOption(this.key, this.fullName);

  static List<StatusOption> get allStatus => [
        StatusOption(AppConstants.PENDING, 'Pendiente'),
        StatusOption(AppConstants.CONFIRMED, 'Confirmado'),
        StatusOption(AppConstants.PROCESSING, 'Procesando'),
        StatusOption(AppConstants.OUT_FOR_DELIVERY, 'En camino'),
        StatusOption(AppConstants.DELIVERED, 'Entregado'),
        StatusOption(AppConstants.RETURNED, 'Devuelto'),
        StatusOption(AppConstants.FAILED, 'Fallido'),
        StatusOption(AppConstants.CANCELLED, 'Cancelado'),
      ];
}
